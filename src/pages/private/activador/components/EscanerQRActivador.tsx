...
interface EscanerQRActivadorProps {
  onScanSuccess: (pin: string) => void;
  onClose: () => void;
  bloqueado: boolean;
}

export const EscanerQRActivador: FC<EscanerQRActivadorProps> = ({
  onScanSuccess,
  onClose,
  bloqueado,
}) => {
  const scannerRef = useRef<Html5Qrcode | null>(null);
  const scannerId = "qr-activador-container";
    ...
    const iniciarEscaner = async () => {
      try {
        ...
        const onScanSuccessCallback = (decodedText: string) => {
          ...
          // Evitar lecturas duplicadas muy seguidas
          if (
          ...
          // Validar que el texto escaneado sea un PIN válido (6 caracteres alfanuméricos)
          ...
          // Pasar el PIN extraído al callback
        ...
        if (err instanceof Error) {
          ...
          if (
            ...
            msg =
              "Permisos de cámara denegados. Habilitá el acceso en tu navegador.";
            ...
            errorMsg.includes("notsupported") ||
  ...
  if (errorCamara) {
    return (
      <div className="rounded-xl border-4 border-danger/30 bg-danger/10 p-8 text-center">
        ...
        <p className="text-xs text-text-tertiary mb-3 whitespace-pre-line">
          {errorCamara}
        </p>
        <button
          onClick={onClose}
      ...
      {bloqueado && (
        <div className="absolute inset-0 bg-success/90 flex items-center justify-center backdrop-blur-sm rounded-xl z-10">
          <div className="text-center text-white">
            ...
            <p className="text-sm">Procesando activación...</p>
          </div>
        </div>
      )}

      {!iniciando && !bloqueado && (
        <div className="absolute top-2 left-2 bg-success/90 text-white text-xs px-3 py-1.5 rounded-full flex items-center gap-2 shadow-lg z-10">
          <div className="w-2 h-2 bg-white rounded-full animate-pulse"></div>
          <span className="font-medium">BUSCANDO QR</span>
        </div>
      )}
      ...
      {!iniciando && !bloqueado && (
        <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/70 text-white text-xs px-3 py-2 rounded-lg text-center z-10">
          ...
          <p className="text-xs opacity-80 mt-1">
            Se activará automáticamente al detectarlo
          </p>
        </div>
      )}
    </div>
