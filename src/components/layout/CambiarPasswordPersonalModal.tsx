...
import { useCambiarPassword } from "../../pages/private/usuarios/hooks/useCambiarPassword";

interface CambiarPasswordPersonalModalProps {
  visible: boolean;
  onClose: () => void;
}

export const CambiarPasswordPersonalModal: FC<CambiarPasswordPersonalModalProps> = ({
  visible,
  onClose,
}) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              >
                {mostrarPasswordActual ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
            {errores.passwordActual && (
              <p className="text-danger text-xs mt-1">{errores.passwordActual}</p>
            )}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary hover:text-text-primary"
              >
                {mostrarPasswordNuevo ? <EyeOff size={16} /> : <Eye size={16} />}
              </button>
            </div>
            {errores.passwordNuevo && (
              <p className="text-danger text-xs mt-1">{errores.passwordNuevo}</p>
            )}
          </div>
